...
namespace live2d 
{
	...
	class LDAllocator;    // LDAllocatorクラスの宣�?
	class MemoryParam ;
	class MemoryInfoSet ;
	class AMemoryHolder ;
	...
	//#if 1
		...
		// ポインタをalignでアライメントする�?signatureSizeバイト�?を�?インタの前に挟�?。戻り�?はsignatureの前に
		...
		// Live2D::dispose() まで生存し続けるメモリグループを取得す�?
	...
	private:
		...
		static MemoryParam*			staticMemoryParam ;// Live2D::init()からLive2D::dispose()まで生�?
		static MemoryParam*			tmpMemoryParam ;// 自由に確保�?開放出来るメモリ?��?常のメモリと同じように使える??
	...
	#ifdef L2D_DEBUG
		static bool					useMemoryInfo ;			// 使用中メモリのログを取る�?合にtrue 
