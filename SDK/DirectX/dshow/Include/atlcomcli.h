...
public:
	...
	T* operator=(__in_opt IUnknown* lp) throw()
	{
        if(*this!=lp)
        {
...
public:
	...
	IUnknown* operator=(__in_opt IUnknown* lp) throw()
	{
        if(*this!=lp)
        {
		    //Actually do a QI to get identity
...
#pragma warning(pop)
	...
	__checkReturn HRESULT CopyTo(__inout_opt BSTR* pbstr) const throw()
	{
...
#pragma warning(pop)
	...
	__checkReturn HRESULT CopyTo(__out_opt VARIANT *pvarDest) const throw()
	{
...
#pragma warning(pop)
	...
	void Attach(__in_opt BSTR src) throw()
	{
		if (m_str != src)
		{
			::SysFreeString(m_str);
...
#pragma warning(pop)
	...
	__checkReturn HRESULT Append(__in_opt LPCSTR lpsz) throw()
	{
		if (lpsz == NULL)
			return S_OK;
	...
	CComBSTR& operator=(__in_opt LPCSTR pSrc)
	{
		::SysFreeString(m_str);
		m_str = A2WBSTR(pSrc);
		if (!*this && pSrc != NULL)
	...
	bool operator<(__in_opt LPCSTR pszSrc) const
	{
		CComBSTR bstr2(pszSrc);
		return operator<(bstr2);
	}
...
public:
	...
	HRESULT CopyTo(__out BSTR *pstrDest) const
	{
