...
Player::Player() {
	SetAnimeModel(_T("Model/Player/Player.X"));
...
void Player::Down::Update() {
	...
	if (time ==  0 || time == 12|| time == 24||time ==36 ||time ==48) {
		_owner->player->damage_efect = true;
	}
	if (time == 6 || time ==18||time ==30 ||time ==42 || time == 54) {
		_owner->player->damage_efect = false;
	}
	...
	_owner->player->anime_speed = 0.016f;
	_owner->player->SetAnimeTrack(3, PLAYERANIMATIONTRACK);
...
void Player::LeftStep::Update() {
	...
	time += 1.f;
	_owner->player->player_position.x -= _owner->player->player_speed * 1.1f;
}
...
void Player::Jump::Update() {
	...
	if (time <= 15) {
		_owner->player->player_position.y += jump_power;
	}
